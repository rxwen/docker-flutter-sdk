...
RUN wget https://github.com/realm/SwiftLint/releases/download/0.63.2/swiftlint_linux_amd64.zip
RUN unzip swiftlint_linux_amd64.zip
RUN mv swiftlint /usr/local/bin
RUN rm -rf swiftlint_linux_amd64.zip LICENSE
...
RUN wget https://github.com/pinterest/ktlint/releases/download/1.8.0/ktlint-1.8.0.zip
RUN unzip ktlint-1.8.0.zip
RUN mv ktlint-1.8.0/bin/ktlint /usr/local/bin
RUN rm -rf ktlint*

ENV SWIFT_FORMAT_VERSION=602.0.0
RUN apt-get update && apt-get install -y curl make git bzip2 xz-utils wget unzip
